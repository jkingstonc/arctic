...
namespace Driver{
    ...
    VGAGraphics VGAGraphics::vga_driver = VGAGraphics();
    ...
    void VGAGraphics::clear(const u8 c){
        colour(c);
    ...
    void VGAGraphics::enable_cursor(u1 enabled=true){
        if(enabled){
    ...
    void VGAGraphics::set_cursor_pos(u32 x, u32 y){
        u16 pos = y * columns + x;
        m_idx = pos;
        IO::outb(0x3d4, 0x0f);
    ...
    u16 VGAGraphics::get_cursor_pos(){
        u16 pos = 0;
        IO::outb(0x3d4, 0x0f);
        pos |= IO::inb(0x3d5);
        IO::outb(0x3d4, 0x0e);
