...
namespace Memory{
    ...
    u32 Heap::alloc(u32 size, u1 page_align){
        ...
        if(page_align){
            ...
            HeapEntryBoundary* heap_footer = (HeapEntryBoundary*)((u32)heap_header+sizeof(HeapEntryBoundary)+size);
            memset(heap_footer, 0, sizeof(HeapEntryBoundary));
            heap_footer->magic=HEAP_ENTRY_MAGIC;
            heap_footer->mode=HEAP_ENTRY_FOOTER;
            heap_footer->hole=HEAP_ENTRY_FULL;
            heap_footer->size=size;

            // now create the new header after the footer
            HeapEntryBoundary* tmp_header = (HeapEntryBoundary*)((u32)heap_footer+sizeof(HeapEntryBoundary));
            ...
            tmp_header->size= found_size - size - sizeof(HeapEntryBoundary)*2;
            ...
            HeapEntryBoundary* original_footer = (HeapEntryBoundary*)((u32)tmp_header+sizeof(HeapEntryBoundary)+ tmp_header->size);
            ASSERT(original_footer->magic==HEAP_ENTRY_MAGIC, "heap entry magic invalid\n");
            ASSERT(original_footer->mode==HEAP_ENTRY_FOOTER, "heap entry isn't footer\n");
            original_footer->size=tmp_header->size;
        }
        ...
        return (u32)heap_header+sizeof(HeapEntryBoundary);
    }
